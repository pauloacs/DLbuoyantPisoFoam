...
def extract_simulation_data(serial, path_to_sims, total_times, deltat, avance, max):
    data = {'Ux': [], 'Uy': [], 'Uz': [],
            ...
            'p_rgh': [], 'delta_p_rgh': [], 'delta_p_rgh_prev': [],
            'delta_rho': [], 'rho':[]}

    for entity in ['p_rgh', 'U_non_cons', 'Cx', 'Cy', 'Cz', 'delta_U', 'delta_p_rgh', 'delta_U_prev', 'delta_p_rgh_prev', 'delta_rho', 'rho']:
      for time in range(int(total_times)):
...
def process_simulation(sim, path_to_sims, avance_list, hdf5_file, total_times, max_sim, max_patch):
    ...
    hdf5_file['sim_data'][sim, ..., 14] = data['delta_p_rgh_prev']
    hdf5_file['sim_data'][sim, ..., 15] = data['delta_rho']
    hdf5_file['sim_data'][sim, ..., 16] = data['rho']
...
def main():
    num_sims_actual = 2
    total_times = 25
    hdf5_path = 'dataset_plate_heat_3d_2sims_25ts_07Apr.hdf5'
    path_to_sims = 'simulation_data'
    max_n_cells_sim = 1000000
