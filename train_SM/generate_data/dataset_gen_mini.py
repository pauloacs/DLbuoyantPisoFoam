...
def extract_simulation_data(serial, path_to_sims, total_times, deltat, avance, max):
    data = {'p_rgh': [], 'Cx': [], 'Cy': [], 'delta_Ux': [], 'delta_Uy': [], 'delta_p_rgh': [], 'delta_rho': []}
    for entity in ['p_rgh', 'Cx', 'Cy', 'delta_U', 'delta_p_rgh', 'delta_rho']:
      for time in range(int(total_times)):
        ...
        if entity in ['delta_U', 'delta_p_rgh']:
            Ux = mesh_cell_data['U_non_cons'][:,0]
            Uy = mesh_cell_data['U_non_cons'][:,1]
            U_max_norm = (np.sqrt(Ux**2 + Uy**2)).max()

        if entity == 'delta_U':
            data['delta_Ux'].append(padding(mesh_cell_data[entity][:,0]/U_max_norm, max))
            data['delta_Uy'].append(padding(mesh_cell_data[entity][:,1]/U_max_norm, max))
        elif entity == 'delta_p_rgh':
            data[entity].append(padding(mesh_cell_data[entity]/U_max_norm**2, max))
        elif entity == 'delta_rho':
            rho = mesh_cell_data['rho']
            rho_max = rho.max()
            data[entity].append(padding(mesh_cell_data[entity]/rho_max, max))
        else:
            extent = 0, 3, 0, 1
            data[entity].append(padding(mesh_cell_data[entity], max))
    return data
...
def create_hdf5_file(hdf5_path, num_sims_actual, total_times, max_n_cells_sim, max_n_cells_patch):
    train_shape = (num_sims_actual, int(total_times), max_n_cells_sim, 7)
...
def process_simulation(sim, path_to_sims, avance_list, hdf5_file, total_times, max_sim, max_patch):
    ...
    hdf5_file['sim_data'][sim, ..., 0] = data['p_rgh']
    hdf5_file['sim_data'][sim, ..., 1] = data['Cx']
    hdf5_file['sim_data'][sim, ..., 2] = data['Cy']
    hdf5_file['sim_data'][sim, ..., 3] = data['delta_Ux']
    hdf5_file['sim_data'][sim, ..., 4] = data['delta_Uy']
    hdf5_file['sim_data'][sim, ..., 5] = data['delta_p_rgh']
    hdf5_file['sim_data'][sim, ..., 6] = data['delta_rho']
...
def main():
    ## this saves the data already non-dimensionalized to save space
    num_sims_actual = 3
    total_times = 10
