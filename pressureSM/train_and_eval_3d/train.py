...
class Training:
  ...
  def __init__(self, grid_res, block_size, var_p, var_in, hdf5_paths, n_samples_per_frame, num_sims, first_t, last_t, standardization_method, chunk_size):
...
####################################################################
  ...
  def define_sample_indexes(self, n_samples_per_frame):
    ...
    for i_sim in range(self.num_sims):
      indices_per_time = []
  ...
  def sample_blocks_chunked(self, sim, t_start, t_end, i_chunk=None, n_chunks=False):
    ...
    for time in range(t_start, t_end):
      ...
      for [ii, jj, kk] in ZYX_indices:
        ...
        inputs_u_sample      = grid[i_idx_fist:i_idx_last, j_idx_first:j_idx_last, k_idx_first:k_idx_last, 0:4]
        inputs_obst_sample   = grid[i_idx_fist:i_idx_last, j_idx_first:j_idx_last, k_idx_first:k_idx_last, 4:5]
        outputs_sample       = grid[i_idx_fist:i_idx_last, j_idx_first:j_idx_last, k_idx_first:k_idx_last, 5:6]
    ...
    inputs_u, inputs_obst, outputs = unique_array[...,0:4], unique_array[...,4:5], unique_array[...,5:6]
  ...
  def sample_blocks(self, sim, t_start, t_end, calculate_maxs=False):
    ...
    if calculate_maxs:
      ...
      self.max_abs_delta_rho = max(np.abs(inputs_u[..., 3]).max(), self.max_abs_delta_rho)
  ...
  def write_sim_fields(self, i):
    ...
    domain_bool, sdf = utils.domain_dist(0, boundaries_list, xyz0)
  ...
  def write_time_step_fields(self, j, data_limited, vert, weights, indices, sdfunct):
    ...
    rho_interp = utils.interpolate_fill(rho_adim, vert, weights)

    # 1D vectors to 3D arrays
    grid = np.zeros(shape=(self.grid_shape_z, self.grid_shape_y, self.grid_shape_x, 6))
    ...
    grid[:,:,:,3:4][tuple(indices.T)] = rho_interp.reshape(rho_interp.shape[0], 1)
    grid[:,:,:,3] = ndimage.gaussian_filter(grid[:,:,:,3], sigma=(5, 5, 5), order=0)

    grid[:,:,:,4:5] = sdfunct
    grid[:,:,:,5:6][tuple(indices.T)] = delta_p_interp.reshape(delta_p_interp.shape[0], 1)
...
####################################################################
  ...
  def write_gridded_simulation_data(self) -> None:
    ...
    self.grid_shape_z = 50
    self.grid_shape_y = 50
    self.grid_shape_x = 500
    ...
    for i in range(self.num_sims):
      print(f"\nProcessing sim {i+1}/{self.num_sims}\n", flush=True)
  ...
  def fit_PCA_to_blocks(self, blocks_data: np.ndarray, client) -> None:
    ...
    x_array_flat = inputs_u.reshape((chunk_size, -1, 4)) / [
        self.max_abs_delta_Ux, self.max_abs_delta_Uy, self.max_abs_delta_Uz, self.max_abs_delta_rho]
  ...
  def fit_PCA(self, chunks_per_sim: int, n_times_per_chunk: int, n_sub_chunks: int) -> None:
    ...
    for sim in range(self.num_sims):
      print(f'Fitting sim {sim+1}/{self.num_sims}...')
      for i_chunk in range(chunks_per_sim):
        ...
        print(f' -Fitting chunk {i_chunk+1}/{chunks_per_sim} for sim {sim+1}/{self.num_sims}', flush = True)
  ...
  def transform_and_write_blocks_to_PC(
    ...
    for sim in range(self.num_sims):
      print(f'Transforming data from sim {sim+1}/{self.num_sims}...')
      for i_chunk in range(chunks_per_sim):
        ...
        print(f' -Transforming chunk {i_chunk+1}/{chunks_per_sim} for sim {sim+1}/{self.num_sims}', flush = True)
        # Create sub-chunks if necessary
        for sub_chunk in range(n_sub_chunks):
          print(f' -Sampling block data for chunk {i_chunk+1}/{chunks_per_sim} - subchunk {sub_chunk+1}/{n_sub_chunks}', flush = True)
          blocks_data = self.sample_blocks_chunked(sim,
...
def main_train_optimized(dataset_path, num_sims, first_t, last_t, num_epoch, lr, beta, batch_size, \
  ...
  Train = Training(grid_res, block_size,var_p, var_in, paths, n_samples_per_frame, num_sims, first_t, last_t, standardization_method, chunk_size)
