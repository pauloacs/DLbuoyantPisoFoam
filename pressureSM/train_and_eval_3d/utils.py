...
def domain_dist(i, boundaries_list, xyz0):
...
def plot_delta_p_comparison(cfd_results, field_deltap, no_flow_bool, slices_indices=[5, 50, 95], fig_path=None):
    ...
    for idx in slices_indices:
        ...
        ax_deltap.plot_surface(X, Y, Z, facecolors=facecolors, rstride=1, cstride=1, alpha=alpha, linewidth=0)
    ...
    for idx in slices_indices:
        ...
        ax_cfd.plot_surface(X, Y, Z, facecolors=facecolors, rstride=1, cstride=1, alpha=alpha, linewidth=0)
    ...
    for idx in slices_indices:
        ...
        ax_error.plot_surface(X, Y, Z, facecolors=facecolors, rstride=1, cstride=1, alpha=alpha, linewidth=0)
...
def plot_inputs_slices(ux, uy, uz, density, deltap, slices_indices=[5, 50, 95], fig_path=None):
    """
    ...
    axs[0, 3].set_title("Density", fontsize=14, fontweight='bold')
    axs[0, 4].set_title("delta_p", fontsize=14, fontweight='bold')
    ...
    for i, idx in enumerate(slices_indices):
        ...
        axs[i, 3].imshow(density[idx, :, :], cmap='viridis', origin='lower')
...
def simple_delta_p_slices_plot():
    # Create the figure and axes
    fig, axs = plt.subplots(5, 3, figsize=(15, 20))

    # Set the column titles
    axs[0, 0].set_title("delta_p predicted", fontsize=14, fontweight='bold')
    axs[0, 1].set_title("CFD results", fontsize=14, fontweight='bold')
    axs[0, 2].set_title("Error (%)", fontsize=14, fontweight='bold')

    # Plot the slices of delta_p predicted
    for i, idx in enumerate(slices_indices):
        ax = axs[i, 0]
        ax.imshow(masked_deltap[idx, :, :], cmap='viridis', origin='lower')
        ax.set_title(f"Slice {idx}/100", fontsize=12, fontweight='bold', loc='left')
        ax.axis("off")

    # Plot the slices of CFD results
    for i, idx in enumerate(slices_indices):
        ax = axs[i, 1]
        ax.imshow(masked_cfd[idx, :, :], cmap='viridis', origin='lower')
        ax.set_title(f"Slice {idx}/100", fontsize=12, fontweight='bold', loc='left')
        ax.axis("off")

    # Plot the slices of the error field
    for i, idx in enumerate(slices_indices):
        ax = axs[i, 2]
        ax.imshow(masked_error[idx, :, :], cmap='viridis', origin='lower')
        ax.set_title(f"Slice {idx}/100", fontsize=12, fontweight='bold', loc='left')
        ax.axis("off")
