...
class Evaluation():
	...
	def assemble_prediction(self, array, indices_list, n_x, n_y, n_z, apply_filter, shape_x, shape_y, shape_z, deltaU_change_grid, deltaP_prev_grid, apply_deltaU_change_wgt):
		...
		for i_block in range(self.x_array.shape[0]):
			...
			flow_bool = self.x_array[i_block,:,:,:,4]
			...
			# 	axs[0].imshow(result[(shape-overlap) * i + 3, :, :])
			# 	axs[1].imshow(result[(shape-overlap) * i + 8, :,:])
			# 	axs[2].imshow(result[(shape-overlap) * i + 12, :,:])
			# 	axs[3].imshow(result[(shape-overlap) * i + 16, :,:])
			# 	axs[4].imshow(result[(shape-overlap) * i + 20,:,:])
			# 	axs[5].imshow(result[(shape-overlap) * i + 25,:,:])
			# 	axs[6].imshow(result[(shape-overlap) * i + 28,:,:])
			# 	for ax in axs:
			# 		plt.colorbar(ax.images[0], ax=ax)
			# 	plt.savefig(f"reconstructed_{i_block}.png")
	...
	def timeStep(self, sim, time, plot_intermediate_fields, save_plots, show_plots, apply_filter):
		...
		delta_rho = data[:self.indice,15:16]
		rho = data[:self.indice,16:17]
		...
		rho_interp = utils.interpolate_fill(rho_adim, self.vert, self.weights)
		p_interp = utils.interpolate_fill(p, self.vert, self.weights)
		...
		grid[0,:,:,:,3:4][tuple(self.indices.T)] = rho_interp.reshape(rho_interp.shape[0], 1)
		grid[0,:,:,:,3] = ndimage.gaussian_filter(grid[0,:,:,:,3], sigma=(5, 5, 5), order=0)

		grid[0,:,:,:,4:5] = self.sdfunct
		grid[0,:,:,:,5:6][tuple(self.indices.T)] = delta_p_interp.reshape(delta_p_interp.shape[0], 1)
		grid[0,:,:,:,6:7][tuple(self.indices.T)] = p_interp.reshape(p_interp.shape[0], 1)
		...
		grid[0,:,:,:,3:4] = grid[0,:,:,:,3:4]/self.max_abs_delta_rho
		grid[0,:,:,:,4:5] = grid[0,:,:,:,4:5]/self.max_abs_dist
		grid[0,:,:,:,5:6] = grid[0,:,:,:,5:6]/self.max_abs_delta_p

		if save_plots:
			utils.plot_inputs_slices(grid[0,...,0], grid[0,...,1], grid[0,...,2], grid[0,...,3], grid[0,...,5], slices_indices=[5, 20, 45])
		...
		for i in range (n_z):
			...
			for j in range (n_y):
				...
				for k in range(n_x):
					...
					x_list.append(grid[0:1, z_0:z_f, y_0:y_f, x_0:x_f, 0:5])
					y_list.append(grid[0:1, z_0:z_f, y_0:y_f, x_0:x_f, 5:6])
		...
		for step in range(y_array.shape[0]):
			y_array[step,...,0][self.x_array[step,...,4] != 0] -= np.mean(y_array[step,...,0][self.x_array[step,...,4] != 0])
		...
		cfd_results = grid[0,:,:,:,5] * self.max_abs_delta_p * pow(U_max_norm,2.0)
		no_flow_bool = grid[0,:,:,:,4] == 0
